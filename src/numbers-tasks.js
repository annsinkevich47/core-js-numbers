...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorAModule = Math.sqrt(x1 ** 2 + y1 ** 2);
  const vectorBModule = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cosA = scalarProduct / (vectorAModule * vectorBModule);
...
function roundToPowerOfTen(num, pow) {
  const pows = 10 ** pow;
  return Math.round(Math.floor(num) / pows) * pows;
}
...
function getSumOfDigits(num) {
  ...
  let sum = arrayOfDigits.reduce(function (a, b) {
    return a + b;
  }, 0);
  return sum;
}
